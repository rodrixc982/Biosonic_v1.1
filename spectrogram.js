...
// desactiva el botón de parada mientras no estás grabando


//configuración del visor: crea el contexto y el lienzo de la API de audio web
...
//bloque principal para grabar audio
...
function callback(stream) {
    ...
    function Plot() {
        ...
        // Establece el color de relleno del lienzo en negro
canvasCtx.fillStyle = "black";

// Registra el tiempo actual en milisegundos como endTime
endTime = performance.now();

// Calcula la diferencia de tiempo entre dos puntos en milisegundos y almacénalo en message3
message3 = "Time between animation frames: " + Math.round((endTime - startTime)).toString() + " ms";

// Llama a la función PlotFFT() para realizar alguna visualización relacionada con FFT
PlotFFT();

// Llama a la función PlotSpectro1() para realizar alguna visualización relacionada con espectrogramas
PlotSpectro1();

// Solicita al navegador que llame a la función Plot antes de la próxima repintura de la ventana, creando una animación continua
...
function zoomIn() {
    ...
    zoomRange.value = parseInt(zoomRange.value) + 10;
    updateZoom();
}
...
function updateZoom() {
    var zoomRange = document.getElementById("zoomRange").value;
    var zoomFactor = zoomRange / 100; // Convertir el valor del rango a un factor de zoom
    var originalFmin = 100; // La frecuencia mínima original que deseas mostrar
    var originalFmax = 5000; // La frecuencia máxima original que deseas mostrar

    // Calcular los nuevos valores de f_min y f_max basados en el factor de zoom
    f_min = originalFmin * zoomFactor;
    f_max = originalFmax * zoomFactor;

    // Recalcular los índices i_min e i_max si es necesario
    i_min = Math.floor(my_X_abs.length * f_min / f_Nyquist);
    i_max = Math.floor(my_X_abs.length * f_max / f_Nyquist);
    ...
    canvas.addEventListener('mousemove', function(event) {
        if (isZooming) {
            endX = event.clientX;
            // Dibuja un rectángulo para mostrar la selección del usuario en tiempo real si es necesario
           // Puedes usar context.rect() y context.stroke() para dibujar el rectángulo.
        }
    });
    ...
    canvas.addEventListener('mouseup', function(event) {
        if (isZooming) {
            endX = event.clientX;
            isZooming = false;
            // Calcula los índices i_min e i_max basados en las coordenadas startX y endX
           // Luego, vuelve a dibujar el espectrograma con el nuevo rango de frecuencias.
          // También, guarda la selección en un archivo de texto usando una función que definiremos a continuación.
           saveSelectionToTextFile(startX, endX);
           updateZoom();
        }
    });


    // Vuelve a dibujar el espectrograma con el nuevo rango de frecuencias
    PlotSpectro1();
}

function saveSelectionToTextFile(startX, endX) {
    // Calcula los índices i_min e i_max basados en las coordenadas startX y endX
    // Aquí, necesitarás convertir las coordenadas X a los índices adecuados para tu espectrograma.
    // Luego, crea un string que contenga los datos de la selección.
    var selectedData = ''; // Aquí debes obtener los datos de tu espectrograma.

    // Crea un objeto Blob para el contenido del archivo de texto.
    var blob = new Blob([selectedData], { type: 'text/plain' });

    // Crea un enlace de descarga para el archivo de texto.
    ...
    // Libera el objeto URL y elimina el enlace del documento.
    URL.revokeObjectURL(url);
    document.body.removeChild(a);
}

let selectionStartX, selectionEndX;

canvas.addEventListener('mousedown', (event) => {
    selectionStartX = event.clientX - canvas.getBoundingClientRect().left;
    // También puedes registrar la coordenada Y si lo necesitas
    let selectionStartX, selectionEndX, selectionStartY, selectionEndY;
    ...
    canvas.addEventListener('mouseup', (event) => {
        selectionEndX = event.clientX - canvas.getBoundingClientRect().left;
        selectionEndY = event.clientY - canvas.getBoundingClientRect().top;
    
        // Ahora, selectionStartX, selectionEndX, selectionStartY y selectionEndY contienen las coordenadas de inicio y fin de la selección en X y Y.
    });
...
canvas.addEventListener('mouseup', (event) => {
    selectionEndX = event.clientX - canvas.getBoundingClientRect().left;
    let canvas = document.getElementById('espectrograma');
...
canvas.addEventListener('mousemove', (event) => {
    if (isMouseDown) {
        ...
        // Dibujar el área de selección en el lienzo (esto es solo un ejemplo visual)
...
canvas.addEventListener('mouseup', () => {
    ...
    // Ahora, selectionStartX, selectionEndX, selectionStartY y selectionEndY contienen las coordenadas de inicio y fin de la selección en X y Y.
    // Puedes usar estos valores para determinar qué parte del espectrograma se seleccionó y realizar las acciones necesarias.
});

});


let startIndex = Math.floor(selectionStartX / bin_width);
let endIndex = Math.floor(selectionEndX / bin_width);
// Ahora, `startIndex` y `endIndex` contienen los índices en tus matrices para la selección.
// Crear un array que contiene los datos seleccionados


let selectedData = my_X_abs.slice(startIndex, endIndex + 1);

// Convertir el array a una cadena de texto
let dataString = selectedData.join('\n');

// Crear un objeto Blob para contener los datos
...
a.href = url;
a.download = 'espectrograma_seleccionado.txt';

navigator.mediaDevices.getUserMedia({ audio: true })
    .then(function (stream) {
        // Tu lógica para manejar el flujo de audio 'stream' aquí
    })
    .catch(function (err) {
...
formulario.addEventListener("submit", function(event) {
    ...
    // Obtiene el archivo de audio seleccionado por el usuario
    var archivoAudio = document.getElementById("archivo_audio").files[0];

    // Crea un objeto FormData para enviar el archivo al servidor
    var formData = new FormData();
    formData.append("archivo_audio", archivoAudio);

    // Realiza una solicitud AJAX para enviar el formulario al servidor
    var xhr = new XMLHttpRequest();
    xhr.open("POST", formulario.action, true);
    xhr.onload = function() {
        // Muestra la respuesta del servidor en el elemento con id "resultado"
