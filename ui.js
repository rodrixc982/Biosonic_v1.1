						...
						action: function ( e ) {
								new PKSimpleModal({
								  ...
								  body:'<label for="k00">Insert url</label>' + 
									'<input style="min-width:250px" placeholder="POR FAVOR INGRESE URL" class="pk_txt" type="text" id="k00" />',
								  setup:function( q ) {
...
btn_clear_selection.innerHTML = '<span>Guardar (Q key)</span>';

// Add click event listener to the button
btn_clear_selection.addEventListener('click', function () {
    // Get the information
    var empezar = document.querySelector('.s_s').textContent;
    var fin = document.querySelector('.s_e').textContent;
    var duracion = document.querySelector('.s_d').textContent;

    // Create a Blob containing the data
    var data = 'Especie De Rana: [Especie aquí]\nInicio del canto: ' + empezar + '\nFin del canto: ' + fin + '\nDuracion del audio señalado: ' + duracion;
    var blob = new Blob([data], { type: 'text/plain' });

    // Create a download link and trigger a click to download the file
    var a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'ranas audio etiquetado.txt';
    a.click();
});

// Append the button to the selection div
selection.appendChild(btn_clear_selection);

// Append the selection div to the document body or another container
